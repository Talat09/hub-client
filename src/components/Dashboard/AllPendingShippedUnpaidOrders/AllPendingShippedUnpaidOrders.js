...
const AllPendingShippedUnpaidOrders = ({ searchText }) => {
  ...
  if (searchText === "pending" || searchText === "shipped") {
    pendingOrShippedOrders = data.filter(
      (order) => order.status === searchText
    );
  } else {
  ...
  const updateToShipped = async (_id) => {
    const { data } = await axiosPrivate.patch(
      `https://parts-bd-inc-server.vercel.app/order/${_id}`
    );
    if (data.success) {
      refetch();
  ...
  return (
    <div className="shadow rounded-xl">
      ...
      {pendingOrShippedOrders.length !== 0 ? (
        ...
        <p className="text-primary text-3xl text-center pb-20 p-5">
          No {searchText} orders found!
        </p>
      )}
      {order && (
