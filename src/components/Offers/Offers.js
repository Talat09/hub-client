...
const Offers = () => {
  ...
  const getParts = async () => {
    const { data } = await axiosPrivate.get(
      `https://parts-master-server.vercel.app/parts?page=0&size=10}`
    );
    return data;
  };
  const { data, isLoading } = useQuery(["all-parts"], getParts, {
  ...
  return (
    <div className="my-20">
      <div className="flex justify-center items-center gap-4 md:gap-10 mt-14 px-6">
        ...
        <h2 className="text-2xl md:text-3xl text-center uppercase font-semibold text-red-500">
          Attention! Deal Zone
        </h2>
        <div>
          ...
          <div className="flex gap-5 mb-6">
            <p
              className={`text-md font-semibold ${
                pathname === "/" && "border-b-2 border-b-primary"
              }`}
            >
